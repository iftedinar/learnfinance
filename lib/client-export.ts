...
function renderResourceHtmlBody(resource: LearningVideo, strategies: LearningAnalysis["strategies"], terms: LearningAnalysis["glossaryTerms"]) {
  return [
    ...
    section("Detailed Study Summary", `<p>${escapeHtml(resource.materials.detailedSummary ?? resource.summary).replace(/\n/g, "<br>")}</p>`),
    section("Main Ideas", list(resource.materials.mainIdeas)),
    section("Study Guide", list(resource.materials.studyGuide)),
    section("Formulas, Models, or Rules", list(resource.materials.formulas ?? [])),
    section("Examples", list(resource.materials.examples ?? [])),
    section("Valuable Lessons", list(resource.materials.valuableLessons ?? [])),
    ...
    section("Strategies and Concepts", strategies.map((strategy) => `<h3>${escapeHtml(strategy.name)}</h3><p>${escapeHtml(strategy.setup)}</p><p><strong>Risk management</strong></p>${list(strategy.riskManagement)}<p><strong>Common mistakes</strong></p>${list(strategy.mistakes)}<p><strong>Checklist</strong></p>${list(strategy.checklist)}`).join("")),
...
function renderResourceText(resource: LearningVideo, analysis: LearningAnalysis) {
  ...
  return [
    ...
    resource.materials.detailedSummary ?? resource.summary,
    "",
    "Main Ideas",
    ...
    "",
    "Formulas, Models, or Rules",
    ...(resource.materials.formulas ?? []),
    "",
    "Examples",
    ...(resource.materials.examples ?? []),
    "",
    "Valuable Lessons",
    ...(resource.materials.valuableLessons ?? []),
    "",
    "Practice Tasks",
    ...
    ...strategies.flatMap((strategy) => [strategy.name, strategy.setup, "Risk Management", ...strategy.riskManagement, "Mistakes", ...strategy.mistakes, "Checklist", ...strategy.checklist, ""]),
    "Glossary",
